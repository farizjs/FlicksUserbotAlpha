...
from userbot import bot, tgbot, LOGS, BOTLOG_CHATID
from userbot.utils import callback
from userbot.utils.log_helper import tag_add, who_tag
...
async def all_messages_catcher(e):
    ...
    if not BOTLOG_CHATID:
        return
    try:
        NEEDTOLOG = BOTLOG_CHATID
    except Exception:
        return LOGS.info("Id Obrolan yang Anda Masukan Salah, Harap Perbaiki")
        ...
        try:
            ...
            if e.photo or e.sticker or e.gif:
                try:
                    ...
                    LOGS.exception(er)
            await tgbot.send_message(NEEDTOLOG, "`Media Tidak Didukung..`", buttons=buttons)
    except (PeerIdInvalidError, ValueError):
        try:
            CACHE_SPAM[NEEDTOLOG]
        except KeyError:
            await tgbot.send_message(
                BOTLOG_CHATID, "Id Obrolan yang Anda Masukan Di Tag Logger Salah, Harap Perbaiki"
            )
            CACHE_SPAM.update({NEEDTOLOG: True})
    except ChatWriteForbiddenError:
        try:
            await tgbot.get_permissions(NEEDTOLOG, "me")
            MSG = "Bot Assistant Anda Tidak Dapat Mengirim Pesan di Grup Tag Log.\nTambahkan untuk Mendapatkan Tag Log Anda.."
        except UserNotParticipantError:
            MSG = "Tambahkan saya ke Grup Log Anda untuk Menyimpan Tag"
        try:
            CACHE_SPAM[NEEDTOLOG]
        except KeyError:
            await tgbot.send_message(LOG_CHANNEL, MSG)
            CACHE_SPAM.update({NEEDTOLOG: True})
    except Exception as er:
        LOGS.exception(er)
...
if BOTLOG_CHATID:
    ...
    @bot.on(events.MessageEdited(func=lambda x: not x.out))
    async def upd_edits(event):
        x = event.sender
        if isinstance(x, User) and (x.bot or x.verified):
            return
        if event.chat_id not in TAG_EDITS:
            if event.sender_id == BOTLOG_CHATID:
                return
            if event.is_private:
                return
        ...
        try:
            MSG = await tgbot.get_messages(BOTLOG_CHATID, ids=d_["id"])
        except Exception as er:
            return LOGS.exception(er)
        TEXT = MSG.text
        if msg:
    ...
    async def idk(e):
        ...
        if chat and msg:
            try:
                await bot.send_message(chat, e.message, reply_to=msg)
...
async def when_added_or_joined(event):
    ...
    if event.user_added:
        tmp = event.added_by
        text = f"#ADD_LOG\n\n{inline_mention(tmp)} baru saja menambahkan {inline_mention(user)} ke {chat}."
    elif event.from_request:
        text = f"#APPROVAL_LOG\n\n{inline_mention(user)} baru saja mendapat Persetujuan Bergabung Ke Obrolan {chat}."
    else:
        text = f"#JOIN_LOG\n\n{inline_mention(user)} baru saja bergabung ke {chat}."
