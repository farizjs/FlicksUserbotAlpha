...
# Recode by @farizjs
# FROM Flicks-Userbot <https://github.com/farizjs/FlicksUserbotAlpha>
...
def get_stuff():
    return sql.gvarstatus("BOTCHAT") or {}


def add_stuff(msg_id, user_id):
    ok = get_stuff()
    ok.update({msg_id: user_id})
    return sql.addgvar("BOTCHAT", ok)
...
def tag_add(msg, chat, user):
    ...
    return sql.gvarstatus("BOTCHAT", ok)
